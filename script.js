$(function(){
  ...
  const CHART_COLORS = { amber:'#FFB020', mint:'#5FC9A8', flag:'#E24A3B', muted:'#9AA6C0', paper:'#F4F2EC', gridline:'rgba(244,242,236,0.08)' };

  // Walks the entered actual paces once and returns everything both the table
  // and the charts need: per-split pace diff (vs planned pace for that split),
  // running actual cumulative time, and cumulative diff vs the planned schedule.
  // Splits left blank are assumed run on-plan so they don't skew later cumulative rows.
  function computeActuals(){
    ...
    return { distLabel, rows, lastFilledIdx };
  }

  function renderCharts(){
    if(!lastSplits || typeof Chart === 'undefined') return;
    const actuals = computeActuals();
    const labels = lastSplits.rows.map(r=>r.label);
    ...
    const paceCfg = {
      ...
      data: { labels, datasets: [
        { label: 'Planned pace', data: plannedPaceData, borderColor: CHART_COLORS.amber, backgroundColor: CHART_COLORS.amber, tension: 0.25, pointRadius: 3, borderWidth: 2 },
        { label: 'Actual pace', data: actualPaceData, borderColor: CHART_COLORS.mint, backgroundColor: CHART_COLORS.mint, tension: 0.25, pointRadius: 3, borderWidth: 2, spanGaps: true }
      ]},
      options: {
        ...
        scales: {
          ...
          y: { ticks: { color: CHART_COLORS.muted, callback: v => fmtPace(v) }, grid: { color: CHART_COLORS.gridline },
               title: { display: true, text: `Pace (${unitSuffix.trim()})`, color: CHART_COLORS.muted } }
        }
      }
    };
    const diffCfg = {
    ...
    if(paceChartInstance){ paceChartInstance.data = paceCfg.data; paceChartInstance.update(); }
    else if(paceCanvas){ paceChartInstance = new Chart(paceCanvas.getContext('2d'), paceCfg); }
    if(diffChartInstance){ diffChartInstance.data = diffCfg.data; diffChartInstance.update(); }
    else if(diffCanvas){ diffChartInstance = new Chart(diffCanvas.getContext('2d'), diffCfg); }
  }

  // Re-sync chart sizing when the Splits tab becomes visible again (canvases in hidden
  // panels can end up with a stale size).
  $('.tab').on('click', function(){
    if($(this).data('panel') === 'splits'){
